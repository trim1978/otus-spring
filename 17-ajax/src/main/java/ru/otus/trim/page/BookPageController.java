...
public class BookPageController {
    ...
    public String getBooksByPage(@RequestParam(name = "page", defaultValue = "0") int page, Model model) {
        ...
        model.addAttribute("total", books.count());
        return "book_list";
    }

    @GetMapping("/book")
    public String getBook(@RequestParam(name = "id", defaultValue = "0") long id, Model model) {
        model.addAttribute("book",id > 0 ? BookDto.toDto (books.findById(id).orElseThrow(NotFoundException :: new)) : new BookDto (0, "", new AuthorDto (0, ""), new GenreDto (0, "")));
//
